...
import os
from utils import gmm_cdf, gamma_mixture_cdf
import joblib

def residual_gmm(params, x, data, eps_data):
    mean1 = params["mean1"]
    mean2 = params["mean2"]
    SD = params["SD"]
    #SD2 = params["SD2"]
...
def fit_res_to_frame_gmm(fit_res, gene, xdata, ydata):
...
def fit_res_to_frame_gamma_mixture(fit_res, gene, xdata, ydata):
    fitparams = fit_res.params.valuesdict()
    df = pd.DataFrame([fitparams])
    df["chisqr"] = fit_res.chisqr
    df["aic"] = fit_res.aic
...
def fit_gaussian_mixture_model(data):
    ...
    """
    # process data
    print("fitting gaussian mixture model..., this is deprecated use gamma mixture instead")
    ...
    for name, group in data.groupby(["gene", "cell_type"]):
        ...
        if len(y) > 3:
            ...
            out = minimize(residual_gmm, params, args=(x, y, eps_data))
            out_df = fit_res_to_frame_gmm(out, group["gene"].iloc[0], x, y)
...
def fit_gamma_mixture_model(data):
    """
    """
    # process data
    print("fitting gamma mixture model")
    data = data.drop(["val_norm_rtm2"], axis=1)
    data = data.drop_duplicates()
